...
const categories: Array<{ label: string; value: ProjectCategory }> = [
  ...
  { label: 'Branding & Packaging', value: 'Brand' },
...
const introductions: Record<ProjectCategory, { title: string; text: string }> = {
  ...
  Brand: { title: 'Branding and packaging that work together.', text: 'From brochures and brand collateral to packaging systems, I create clear, connected communication that gives every format a recognisable identity.' },
...
export function WorkArchive({ projects }: { projects: Project[] }) {
  ...
  const matchesCategory = (project: (typeof projects)[number], selectedCategory: ProjectCategory) =>
    selectedCategory === 'Brand'
      ? project.category === 'Brand' || project.category === 'Packaging'
      : project.category === selectedCategory;
  const categoryProjects = projects.filter((project) => matchesCategory(project, category));
      ...
      <div className="archive-projects" id="project-list" ref={projectListRef}>
        <div className="archive-navigation">
          <div className="category-tabs" aria-label="Project categories">
            {categories.map((item) => {
              const count = projects.filter((project) => matchesCategory(project, item.value)).length;
